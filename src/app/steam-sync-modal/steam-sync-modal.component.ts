...
export class SteamSyncModalComponent {
  ...
  async checkSteamId(): Promise<void> {
    ...
    try {
      this.steamUser = await lastValueFrom(
        this.userService.getSteamUserData(this.steamIdFormControl.value),
      );
    } catch (e) {
  ...
  async synchronizeSteamLibrary(): Promise<void> {
    ...
    try {
      await lastValueFrom(
        this.gameService.synchronizeSteamAccount(this.steamIdFormControl.value),
      );
      ...
      this.snackBar.open(
        this.translate.instant('steamSync.snackBarSuccess'),
        '',
        { duration: 3000, panelClass: ['blue-snackbar'] },
      );
      this.steamSynctDialogRef.close({
        avatar: this.steamUser?.avatar,
        changeAvatar: this.wantToKeepSteamAvatar && this.steamUser?.avatar,
      });
    } catch (e) {
      throw e;
    } finally {
